...
const liveStatsGate = document.getElementById('liveStatsGate');
const liveStatsChat = document.getElementById('liveStatsChat');
...
// Connect once, right away, so the "people online now" count works even
// before someone has accepted the age gate. Joining the chat queue only
// happens later, on demand, over this same connection.
const socket = io();

socket.on('stats', ({ online, activeChats }) => {
  const gateMsg = `${online} ${online === 1 ? 'person' : 'people'} online now` +
    (activeChats > 0 ? ` · ${activeChats} ${activeChats === 1 ? 'chat' : 'chats'} in progress` : '');
  liveStatsGate.textContent = gateMsg;
  liveStatsChat.textContent = `${online} online`;
});
...
socket.on('disconnect', () => {
  ...
  liveStatsGate.textContent = 'Disconnected — reload to reconnect';
});
...
enterBtn.addEventListener('click', () => {
  ...
  if (socket.connected) {
    ...
    socket.once('connect', () => socket.emit('join_queue'));
  }
});
